-- ============================================================
-- SQLite-safe reference answers, used ONLY by run_tests.py to grade
-- myanswers.sql. Semantically identical to answers.sql (Postgres),
-- with the handful of Postgres-only functions swapped per
-- SQLITE_NOTES.md. Read answers.sql for the "canonical"/readable
-- version — this file exists purely so grading actually runs.
--
-- Questions 66, 67 are prose/design questions with no single graded
-- result set — run_tests.py marks them INFO (ungraded) automatically.
-- Question 65 (CREATE VIEW) is graded structurally (executes OK),
-- not by row comparison.
-- ============================================================
...
SELECT employee_id, order_id, order_date,
       julianday(order_date) - julianday(LAG(order_date) OVER (PARTITION BY employee_id ORDER BY order_date)) AS days_since_previous
FROM orders;

-- 45
SELECT order_id, julianday(ship_date) - julianday(order_date) AS days_to_ship
FROM orders
...
WHERE strftime('%Y-%m', c.signup_date) = strftime('%Y-%m', f.first_order_date);

-- 49
SELECT order_id, order_date, ship_date, julianday(ship_date) - julianday(order_date) AS days_to_ship
FROM orders
WHERE ship_date IS NOT NULL AND julianday(ship_date) - julianday(order_date) > 5;

-- 50
SELECT employee_id, AVG(julianday(ship_date) - julianday(order_date)) AS avg_shipping_delay
FROM orders
...
-- 52
SELECT strftime('%w', order_date) AS day_of_week, COUNT(*) AS num_orders
FROM orders
...
-- 54
SELECT customer_id, email, substr(email, instr(email, '@') + 1) AS email_domain
FROM customers;
...
WHERE julianday(d.max_date) - julianday(l.last_order_date) > 60;
...
WITH active_monthly AS (
    SELECT strftime('%Y-%m', order_date) AS month, COUNT(DISTINCT customer_id) AS active_customers
    FROM orders
